...
function useMorphContext(component) {
 ...
 */
export function MorphPopover({ children, open: controlledOpen, defaultOpen = false, onOpenChange, className }) {
    const baseId = useId();
    ...
    const anchorRef = useMemo(() => ({ current: trigger ?? root }), [root, trigger]);
    ...
    return (<MorphContext.Provider value={ctx}>
...
export function MorphPopoverTrigger({ children }) {
...
const originFor = (side, align) => `${side === "bottom" ? "top" : "bottom"} ${align === "end" ? "right" : "left"}`;
...
function clipHidden(side, align, radius) {
...
const clipShown = (radius) => `inset(0% 0% 0% 0% round ${radius}px)`;
...
const MORPH_CLIP_TRANSITION = {
    duration: 0.32,
    ease: EASE_OUT
};
export function MorphPopoverContent({ children, side = "bottom", align = "end", sideOffset = 8, radius = 16, className }) {
    ...
    {ctx.open ? (<motion.div data-morph-popover-portal="" 
        ...
        variants={wrap} initial={reduce ? { opacity: 0 } : "hidden"} animate={reduce ? { opacity: 1 } : "show"} exit={reduce ? { opacity: 0 } : "hidden"} transition={reduce ? { duration: 0.12 } : undefined} style={{
                left,
                top,
                visibility: layout ? "visible" : "hidden",
                transformOrigin: originFor(side, align),
            }} className="fixed z-[9999] [filter:drop-shadow(0_10px_18px_rgba(0,0,0,0.14))]">
        <motion.div ref={ctx.contentRef} id={ctx.contentId} role="dialog" aria-labelledby={ctx.triggerId} variants={clip} style={{ borderRadius: radius }} className={cn("overflow-hidden border border-border bg-background", className)}>
          {children}
        </motion.div>
      </motion.div>) : null}
