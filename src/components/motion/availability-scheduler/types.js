 ...
 */
export const panelKey = (day, rangeId, edge) => `${day}:${rangeId}:${edge}`;
// ─── time helpers ────────────────────────────────────────────────────────────
export const toMinutes = (v) => {
    const [h, m] = v.split(":").map(Number);
    return h * 60 + m;
};
export const toValue = (mins) => {
    const clamped = Math.max(0, Math.min(24 * 60 - 1, mins));
    const h = Math.floor(clamped / 60);
    const m = clamped % 60;
    return `${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}`;
};
export const label12 = (v) => {
    const [h, m] = v.split(":").map(Number);
    const ap = h < 12 ? "AM" : "PM";
    const h12 = h % 12 === 0 ? 12 : h % 12;
    return `${h12}:${String(m).padStart(2, "0")} ${ap}`;
};
export function buildOptions(step) {
    const out = [];
...
function snapToOption(mins, slots) {
    let best = slots[0];
    for (const slot of slots) {
        if (Math.abs(slot - mins) < Math.abs(best - mins))
            best = slot;
    }
    return best;
}
function withCurrentOption(filtered, all, current) {
    if (!current || filtered.some((o) => o.value === current))
        return filtered;
    const extra = all.find((o) => o.value === current) ??
        ({
            value: current,
            label: label12(current)
        });
 ...
 */
export function clampRange(start, end, options, changed = "start") {
...
export function startOptions(options, end, current) {
...
export function endOptions(options, start, current) {
