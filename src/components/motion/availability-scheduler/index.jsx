...
import { buildOptions, defaultWeek, panelKey, WEEKDAYS } from "./types";
export { defaultWeek } from "./types";
export function AvailabilityScheduler({ value, defaultValue, onChange, step = 30, className }) {
    const reduce = useReducedMotion() ?? false;
    const groupId = useId();
    const options = useMemo(() => buildOptions(step), [step]);
    const idRef = useRef(0);
    const [internal, setInternal] = useState(() => defaultValue ?? defaultWeek());
    ...
    useEffect(() => {
        if (openPanel !== null && !livePanels.has(openPanel))
            setOpenPanel(null);
    ...
    const setDay = useCallback((day, next) => {
        commit({ ...week, [day]: next });
    }, [commit, week]);
    const panelOpenChange = useCallback((day, id, open) => {
    ...
    const copyDay = useCallback((from, targets) => {
    ...
    return (<LayoutGroup id={groupId}>
      <div className={cn("w-full divide-y divide-border", className)}>
        {WEEKDAYS.map(({ key, label }) => (<DayRow key={key} day={key} label={label} state={week[key]} options={options} reduce={reduce} elevated={openDay === key} openPanel={openPanel} onChange={(next) => setDay(key, next)} onCopy={(targets) => copyDay(key, targets)} onPanelOpenChange={(id, open) => panelOpenChange(key, id, open)}/>))}
      </div>
    </LayoutGroup>);
}
