...
let lastHiddenAt = 0;
export function Tooltip({ content, children, side = "top", delay = 120, className, wrapperClassName }) {
    ...
    const variants = useMemo(() => buildVariants(side), [side]);
    ...
    return (<>
      ...
      <span ref={anchorRef} className={cn("relative inline-flex align-middle", wrapperClassName)} 
    ...
    onPointerEnter={(event) => {
            ...
            if (hover.leave(event))
                hide();
        }} onFocus={show} onBlur={hide} onPointerDown={(event) => tap.start(event, open)} 
    ...
    // since lifted.
    onPointerCancel={tap.drop} onKeyDown={tap.drop} onClick={toggleOnTap}>
        {trigger}
      </span>
      {typeof document !== "undefined"
            ? createPortal(<AnimatePresence>
        {open && coords ? (<span aria-hidden className="pointer-events-none fixed z-9999" style={{
                        top: coords.top,
                        left: coords.left,
                        transform: anchorTransform[side],
                    }}>
            <motion.span id={id} role="tooltip" variants={variants} initial="initial" animate="animate" exit="exit" style={{ transformOrigin: transformOrigin[side] }} className={cn("block whitespace-nowrap rounded-lg border border-border bg-background px-2.5 py-1 text-xs font-medium text-foreground shadow-ultimate", className)}>
              {content}
            </motion.span>
          </span>) : null}
