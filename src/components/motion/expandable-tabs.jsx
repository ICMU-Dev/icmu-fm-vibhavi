...
export function ExpandableTabs({
  ...
  const getActiveTabWidth = useCallback((item) =>
    Math.max(TAB_W, ACTIVE_LEFT_PAD +
      ICON_W +
      LABEL_GAP +
  ...
  return (
    <>
      <motion.div
        ...
        className={cn(
          "relative overflow-hidden rounded-[26px] border border-border bg-card shadow-ultimate",
          className,
          classNames?.root
        )}>
        <div
          ...
          {items.map((item) => {
            ...
            return (
              <motion.button
                ...
                className={cn(
                  "relative isolate flex h-9 min-w-8 shrink-0 items-center justify-center overflow-hidden rounded-[18px] px-2 text-sm font-bold outline-none",
                ...
                {isActive ? (
                  <span
                    className={cn("absolute inset-0 -z-10 rounded-[18px] bg-foreground/10", classNames?.pill)} />
                ) : null}
      ...
      <div
        ...
        {items.map((item) => (
          <span
            className={cn("whitespace-nowrap text-sm font-bold leading-none", classNames?.label)}
            key={item.id}
