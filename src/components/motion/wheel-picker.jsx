...
const BACK = 1.35;
const easeOutBack = (p) => 1 + (BACK + 1) * (p - 1) ** 3 + BACK * (p - 1) ** 2;
...
export function WheelPicker({ options, value, defaultValue, onValueChange, visibleCount = 5, itemHeight = 36, disabled = false, sound = false, className, "aria-label": ariaLabel }) {
    ...
    const paint = useCallback((s) => {
        ...
        if (drum) {
            ...
            for (const node of Array.from(drum.children)) {
                ...
                if (li.style.visibility !== want)
                    li.style.visibility = want;
            }
        }
    ...
    const maybeTick = useCallback((pos) => {
    ...
    const glide = useCallback((to, duration, ease = easeOutCubic) => {
        stop();
    ...
    const fling = useCallback((velocity) => {
        const from = scroll.current;
        if (from < 0 || from > last) {
    ...
    const step = useCallback((by) => glide(clamp(Math.round(scroll.current) + by, 0, last), 300, easeOutBack), [glide, last]);
    ...
    const onPointerDown = useCallback((event) => {
        if (disabled || reduce || event.pointerType === "touch")
            return;
    ...
    const maskFade = "[mask-image:linear-gradient(to_bottom,transparent,#000_22%,#000_78%,transparent)]";
    if (reduce) {
        const pad = (height - itemHeight) / 2;
        return (<div className={cn("relative overflow-hidden rounded-2xl border border-border bg-card", disabled && "pointer-events-none opacity-50", className)} style={{ height }}>
        <div className="pointer-events-none absolute inset-x-0 top-1/2 z-10 -translate-y-1/2 border-border border-y bg-foreground/[0.04]" style={{ height: itemHeight }}/>
        <ul className={cn("h-full snap-y snap-mandatory overflow-y-auto scroll-smooth [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden", maskFade)} style={{ paddingTop: pad, paddingBottom: pad }}>
          {options.map((option) => {
                const v = optionValue(option);
                return (<li key={v} className="snap-center">
                <button type="button" disabled={disabled} onClick={() => emit(options.indexOf(option))} className={cn("flex w-full items-center justify-center font-medium", v === currentValue
                        ? "text-foreground"
                        : "text-muted-foreground")} style={{ height: itemHeight }}>
                  {optionLabel(option)}
                </button>
              </li>);
            })}
    ...
    return (<div ref={container} role="listbox" aria-label={ariaLabel} tabIndex={disabled ? -1 : 0} onKeyDown={onKeyDown} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} className={cn("relative touch-none overflow-hidden rounded-2xl border border-border bg-card outline-none focus-visible:ring-2 focus-visible:ring-foreground/20", 
        // The drum owns the whole gesture; iOS must not open its callout or
        // start a drag out of the same press and cancel ours.
        TOUCH_GESTURE_CLASS, grabbing ? "cursor-grabbing" : "cursor-grab", disabled && "pointer-events-none opacity-50", maskFade, className)} style={{ height, perspective: 1000 }}>
      {/* Curved drum of dimmed rows. */}
      <ul ref={drumRef} aria-hidden className="absolute inset-x-0 top-1/2 m-0 h-0 list-none p-0 [backface-visibility:hidden] [transform-style:preserve-3d] [will-change:transform]">
        {options.map((option, i) => (<li key={optionValue(option)} data-index={i} className="absolute inset-x-0 flex items-center justify-center font-medium text-muted-foreground" style={{
                top: -itemHeight / 2,
                height: itemHeight,
      ...
      <div className="pointer-events-none absolute inset-x-0 top-1/2 z-10 -translate-y-1/2 overflow-hidden rounded-md bg-foreground/[0.04]" style={{ height: itemHeight, perspective: 1000 }}>
        <ul ref={bandRef} aria-hidden className="absolute inset-x-0 top-1/2 m-0 h-0 list-none p-0 [backface-visibility:hidden] [transform-style:preserve-3d] [will-change:transform]">
          {options.map((option, i) => (<li key={optionValue(option)} data-index={i} className="absolute inset-x-0 flex items-center justify-center font-medium text-foreground" style={{
                top: -itemHeight / 2,
                height: itemHeight,
                transform: `rotateX(${-itemAngle * i}deg) translateZ(${radius}px)`,
            }}>
