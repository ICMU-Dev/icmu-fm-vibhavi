...
export function RangeSlider({ showTicks = true, className, ...options }) {
    ...
    const ticks = showTicks && steps > 0 && steps <= 50
        ...
        : [];
    return (<div {...trackProps} className={cn("relative flex h-10 w-full touch-none items-center overflow-hidden rounded-lg bg-muted", TOUCH_GESTURE_CLASS, options.disabled
      ...
      <div className="pointer-events-none absolute inset-x-0.75 inset-y-0">
        {ticks.map((t) => {
            const tp = ((t - min) / (max - min)) * 100;
            return (<span key={t} className="absolute top-1/2 size-1 -translate-x-1/2 -translate-y-1/2 rounded-full bg-foreground/25" style={{ left: `${tp}%` }}/>);
        })}
      </div>

      {/* vertical bar thumb — contained at both ends via thumbX */}
      <motion.div {...sliderProps} animate={reduce ? undefined : { scaleY: dragging ? 1.35 : 1 }} transition={SPRING_BOUNCY} className="absolute top-1/2 h-5 w-1.5 rounded-sm bg-foreground shadow-sm outline-none ring-inset ring-foreground/30 focus-visible:ring-4" style={{ left, x: thumbX, y: "-50%" }}/>
    </div>);
}
