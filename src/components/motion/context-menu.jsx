...
import { cloneElement, createContext, isValidElement, useCallback, useContext, useEffect, useId, useLayoutEffect, useMemo, useRef, useState, } from "react";
...
function getEnabledItems(container) {
    if (!container)
        return [];
    return Array.from(container.querySelectorAll('[data-context-menu-item="true"]:not([data-disabled="true"])'));
}
function clamp(value, min, max) {
    return Math.min(Math.max(value, min), max);
}
function collapsedClip(origin, size) {
    const half = 8;
...
export function ContextMenu({ children, open: controlledOpen, defaultOpen = false, onOpenChange, className }) {
    ...
    const openAt = useCallback((nextPoint, nextModality) => {
        setPoint(nextPoint);
...
export function ContextMenuTrigger({ children, disabled = false, className }) {
    const context = useContextMenuContext("ContextMenuTrigger");
    const longPressTimer = useRef(null);
    const touchOrigin = useRef(null);
    const releaseSelection = useRef(null);
    ...
    const onPointerDown = (event) => {
        ...
        if (event.defaultPrevented || disabled || !pressToOpen)
            return;
    ...
    const onKeyDown = (event) => {
        ...
        context.openAt({ x: rect.left + Math.min(24, rect.width / 2), y: rect.top + rect.height / 2 }, "keyboard");
    };
...
export function ContextMenuContent({ children, className, ariaLabel = "Context menu" }) {
    const context = useContextMenuContext("ContextMenuContent");
    const [mounted, setMounted] = useState(false);
    const [position, setPosition] = useState(context.point);
    ...
    useLayoutEffect(() => {
        ...
        const left = Math.max(VIEWPORT_PADDING, Math.min(Math.max(context.point.x, VIEWPORT_PADDING), window.innerWidth - rect.width - VIEWPORT_PADDING));
        const top = Math.max(VIEWPORT_PADDING, Math.min(Math.max(context.point.y, VIEWPORT_PADDING), window.innerHeight - rect.height - VIEWPORT_PADDING));
    ...
    const onKeyDown = (event) => {
        ...
        if (event.key.length === 1 &&
            ...
            const match = getEnabledItems(context.contentRef.current).find((item) => (item.dataset.label ?? item.textContent ?? "")
                .trim()
                .toLocaleLowerCase()
    ...
    return createPortal(<div data-context-menu-portal="" aria-hidden={!context.open} inert={!context.open} style={{ left: position.x, top: position.y }} className={cn("fixed z-[100] [filter:drop-shadow(0_18px_28px_rgba(0,0,0,0.2))]", context.open ? "pointer-events-auto" : "pointer-events-none")}>
    <motion.div ref={context.contentRef} id={context.menuId} role="menu" aria-label={ariaLabel} data-morph-ready={morphReady ? "true" : "false"} tabIndex={-1} initial={false} animate={{
            opacity: visualOpen ? 1 : 0,
            clipPath: context.reduce || context.modality === "keyboard" || visualOpen
                ? clipShown
                : clipHidden,
        }} transition={context.modality === "keyboard"
            ? { duration: 0 }
            : context.reduce
                ...
                }} onKeyDown={onKeyDown} onContextMenu={(event) => event.preventDefault()} className={cn("min-w-56 overflow-hidden rounded-xl border border-border bg-card p-1.5 text-foreground outline-none", className)}>
      {children}
    </motion.div>
  </div>, document.body);
}
function ContextMenuItemBase({ children, onSelect, disabled = false, closeOnSelect = true, tone = "default", inset = false, className, textValue, role = "menuitem", ariaChecked }) {
    ...
    return (<button type="button" id={id} role={role} {...checkedProps} disabled={disabled} data-context-menu-item="true" data-disabled={disabled ? "true" : undefined} data-label={textValue} tabIndex={-1} onFocus={() => context.setActiveId(id)} onPointerMove={(event) => {
            ...
            if (closeOnSelect)
                context.setOpen(false);
        }} className={cn("relative isolate flex w-full select-none items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-[13px] outline-none", "focus-visible:ring-2 focus-visible:ring-foreground/15", "disabled:pointer-events-none disabled:opacity-40", inset && "pl-8", tone === "destructive" ? "text-destructive" : "text-foreground", className)}>
      {active ? (<motion.span layoutId={`${context.menuId}-active`} className={cn("absolute inset-0 -z-10 rounded-lg", tone === "destructive"
                ? "bg-destructive/10"
                : "bg-foreground/[0.065]")} transition={context.reduce ? { duration: 0 } : SPRING_LAYOUT}/>) : null}
      {children}
    </button>);
}
...
export function ContextMenuCheckboxItem({ checked, onCheckedChange, children, ...props }) {
    const context = useContextMenuContext("ContextMenuCheckboxItem");
    return (<ContextMenuItemBase {...props} role="menuitemcheckbox" ariaChecked={checked} onSelect={() => onCheckedChange?.(!checked)}>
      <span className="flex h-4 w-4 shrink-0 items-center justify-center">
        <AnimatePresence initial={false}>
          {checked ? (<motion.span key="check" initial={context.reduce ? false : { opacity: 0, scale: 0.75 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: context.reduce ? 1 : 0.75 }} transition={context.reduce ? { duration: 0.08 } : SPRING_PANEL}>
...
export function ContextMenuRadioGroup({ value, onValueChange, children, className }) {
    const context = useMemo(() => ({ value, onValueChange }), [value, onValueChange]);
    return (<ContextMenuRadioGroupContext.Provider value={context}>
      <div className={className}>{children}</div>
    </ContextMenuRadioGroupContext.Provider>);
}
export function ContextMenuRadioItem({ value, children, ...props }) {
    ...
    return (<ContextMenuItemBase {...props} role="menuitemradio" ariaChecked={checked} onSelect={() => group.onValueChange?.(value)}>
      <span className="flex h-4 w-4 shrink-0 items-center justify-center">
        <span className={cn("h-1.5 w-1.5 rounded-full bg-current transition-opacity", checked ? "opacity-100" : "opacity-0")}/>
      </span>
      {children}
    </ContextMenuItemBase>);
}
export function ContextMenuLabel({ children, inset = false, className }) {
    return (<div className={cn("px-2.5 pb-1 pt-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground", inset && "pl-8", className)}>
      {children}
    </div>);
}
...
export function ContextMenuShortcut({ children, className }) {
    return (<span aria-hidden="true" className={cn("ml-auto pl-4 text-[10px] font-medium tracking-wide text-muted-foreground", className)}>
      {children}
    </span>);
}
