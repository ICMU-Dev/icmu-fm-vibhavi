...
const isEnabled = (enabledItems, candidate) => candidate !== undefined && enabledItems.some((i) => i.value === candidate);
/**
 ...
 */
function liveCursorValue(cursor, options) {
    if (cursor === null || cursor.query !== options.query)
        return null;
 ...
 */
function fallbackActive({ value, enabledItems }) {
...
const resolveActive = (cursor, options) => liveCursorValue(cursor, options) ?? fallbackActive(options);
export function useActiveOption({ open, ...options }) {
