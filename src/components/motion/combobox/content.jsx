...
export function ComboboxContent({ children, side = "bottom", align = "start", sideOffset = 6, avoidCollisions = true, className }) {
    ...
    return createPortal(<motion.div ref={context.contentRef} data-combobox-content="" data-side={actualSide} aria-hidden={!context.open} inert={!context.open} initial={false} animate={{
            ...
            y: context.open
                ? actualSide === "bottom"
                    ...
                    : -sideOffset
                : 0,
        }} transition={context.reduce || !morphReady ? { duration: 0 } : COMBOBOX_MORPH} style={{
            left,
            top: actualSide === "bottom" && layout
                ? layout.trigger.top + layout.trigger.height
                : undefined,
            bottom: actualSide === "top" && layout
        ...
        }} className={cn("fixed z-[9999] w-(--combobox-trigger-width) overflow-hidden rounded-xl border border-border bg-background text-popover-foreground outline-none will-change-[height,transform]", className)}>
    <motion.div ref={measureRef} initial={false} animate={{ opacity: context.open ? 1 : 0 }} transition={context.reduce || !morphReady ? { duration: 0 } : COMBOBOX_MORPH}>
      {children}
    </motion.div>
