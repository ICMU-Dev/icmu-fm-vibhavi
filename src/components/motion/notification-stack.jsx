  ...
  return (
    <span className={cn("flex min-w-0 flex-col gap-1.5 py-4", classNames?.content)}>
      <span className="flex min-w-0 items-start justify-between gap-3">
        <span
          className={cn("min-w-0 text-sm font-medium leading-snug", classNames?.title)}>
          {item.title}
        </span>
        {item.trailing ? (
...
export function NotificationStack({
  ...
  if (!primaryItem) {
    return (
      <div
        className={cn(
          "flex w-full max-w-88 items-center justify-center gap-2 rounded-3xl bg-muted/70 px-5 py-8 text-sm font-medium text-muted-foreground",
          className
        )}>
  ...
  return (
    <motion.button
      ...
      onClick={handleClick}
      className={cn(
        "relative z-10 block w-full max-w-88 cursor-pointer rounded-3xl text-left text-foreground outline-none",
      ...
      <span className="absolute inset-x-0 bottom-0 block p-3">
        ...
        <span
          className={cn("relative z-10 grid gap-1", !isExpanded && "pb-2", classNames?.stack)}>
