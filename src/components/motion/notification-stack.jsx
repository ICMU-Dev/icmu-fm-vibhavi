...
function NotificationCardContent({ item, classNames }) {
    return (<span className={cn("flex min-w-0 flex-col gap-1.5 py-4", classNames?.content)}>
      <span className="flex min-w-0 items-start justify-between gap-3">
        <span className={cn("min-w-0 text-sm font-medium leading-snug", classNames?.title)}>
          {item.title}
        </span>
        {item.trailing ? (<span className={cn("shrink-0 text-xs", classNames?.trailing)}>
            {item.trailing}
          </span>) : null}
      </span>
      {item.description ? (<span className={cn("text-xs leading-relaxed text-muted-foreground", classNames?.description)}>
          {item.description}
        </span>) : null}
    </span>);
}
export function NotificationStack({ items, expanded, defaultExpanded = false, onExpandedChange, onViewAll, maxVisible = 3, collapsedLabel = "Notifications", expandedLabel = "View all", emptyLabel = "All caught up", className, classNames }) {
    ...
    if (!primaryItem) {
        return (<div className={cn("flex w-full max-w-88 items-center justify-center gap-2 rounded-3xl bg-muted/70 px-5 py-8 text-sm font-medium text-muted-foreground", className)}>
    ...
    return (<motion.button ref={rootRef} type="button" initial={false} aria-expanded={isExpanded} aria-label={isExpanded
    ...
    onPointerEnter={(event) => {
            ...
            if (hover.leave(event) && !hasFocus.current)
                collapse();
        }} onPointerDown={(event) => {
    ...
    onPointerCancel={tap.drop} onFocus={() => {
            hasFocus.current = true;
            setIsExpanded(true);
        }} onBlur={handleBlur} onKeyDown={handleKeyDown} onClick={handleClick} className={cn("relative z-10 block w-full max-w-88 cursor-pointer rounded-3xl text-left text-foreground outline-none", "focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background", className)}>
      ...
      <span className="absolute inset-x-0 bottom-0 block p-3">
        <motion.span aria-hidden="true" layout initial={false} transition={backgroundTransition} className="absolute inset-0 rounded-3xl bg-muted"/>
        <span className={cn("relative z-10 grid gap-1", !isExpanded && "pb-2", classNames?.stack)}>
          {visibleItems.map((item, index) => {
            const isPrimary = index === 0;
            return (<motion.span key={item.id} layout="position" initial={false} animate={{
                    y: isExpanded ? 0 : index * STACK_PEEK,
                    clipPath: isExpanded
                        ? "inset(0px 0px round 16px)"
                        : `inset(0px ${index * STACK_INSET}px round 16px)`,
                }} transition={cardTransition} className={cn("block rounded-2xl border border-border/60 bg-background px-4", classNames?.card)} style={{
        ...
        <motion.span layout="position" transition={transition} className={cn("relative z-10 mt-2 flex min-h-9 items-center gap-2 px-1", classNames?.footer)}>
          <span className={cn("grid size-7 shrink-0 place-items-center rounded-full bg-orange-600 text-xs font-medium text-white shadow-[inset_0_1px_2px_rgb(0_0_0/0.2),inset_0_-1px_0_rgb(255_255_255/0.16)] dark:bg-orange-500", classNames?.count)}>
            {items.length}
          </span>
          <span className="flex items-center text-sm font-medium">
            <ActionSwapText value={isExpanded ? "expanded" : "collapsed"} animation="roll">
              {isExpanded ? (<span className="inline-flex items-center gap-1">
                  {expandedLabel}
